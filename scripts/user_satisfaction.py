...
class UserStatisfactionCalculator:
    ...
    columns_of_experience = ["MSISDN/Number", "Avg RTT DL (ms)", "Avg RTT UL (ms)", "TCP DL Retrans. Vol (Bytes)", "TCP UL Retrans. Vol (Bytes)", "Handset Type", "Avg Bearer TP DL (kbps)", "Avg Bearer TP UL (kbps)"]
    
    # columns used/aggregated to track the user engagement
    columns_of_engagement = ["MSISDN/Number", "Start", "Start ms", "End", "End ms", "Dur. (ms)", "Dur. (ms).1"]

    def __init__(self, data: pd.DataFrame):
        self.data = data
        self.engagement_clusters = None
        self.experience_clusters = None
